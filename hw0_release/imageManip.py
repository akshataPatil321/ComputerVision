...
def resize_image(input_image, output_rows, output_cols):
    ...
    row_sf = (input_rows/output_rows)
    col_sf = (input_cols/output_cols)
    ...
    for i in range(output_rows):
        for j in range(output_cols):
            input_i = int(i * row_sf)
            input_j = int(j * col_sf)
...
def rotate_image(input_image, theta):
    ...
    for i in range(output_rows):
        for j in range(output_cols):
            ...
            rotated_i, rotated_j = rotate2d(np.array([i - center_i, j - center_j]), theta)
            # Converting back to indices:
            rotated_i, rotated_j = int(rotated_i + center_i), int(rotated_j + center_j)
            # Check if the indices are within bounds
            if 0 <= rotated_i < input_rows and 0 <= rotated_j < input_cols:
                output_image[i, j, :] = input_image[rotated_i, rotated_j, :]      
    ### END YOUR CODE
